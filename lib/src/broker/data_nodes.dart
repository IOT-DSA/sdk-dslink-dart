...
class BrokerDataNode extends BrokerNode {
  ...
  BrokerNode parent;
  BrokerDataNode(String path, BrokerNodeProvider provider) : super(path, provider) {
  ...
  Response setValue(Object value, Responder responder, Response response,
    ...
    if (storage != null && (lastValueUpdate == null || lastValueUpdate.value != value)) {
...
class BrokerDataRoot extends BrokerDataNode {
  BrokerNode parent;
  BrokerDataRoot(String path, BrokerNodeProvider provider) : super(path, provider) {
...
InvokeResponse addDataNode(Map params, Responder responder,
  ...
  if (parentNode is BrokerDataNode &&
    ...
    BrokerDataNode node = responder.nodeProvider.getOrCreateNode('${parentNode.path}/$name', false);
    if (type is String && const['string','number','bool','array','map','binary','dynamic'].contains(type)) {
    ...
    DsTimer.timerOnceBefore((responder.nodeProvider as BrokerNodeProvider).saveDataNodes, 1000);
  }
...
Map dataNodeFunctions = {
  "broker": {
    "dataNode": {"addNode": addDataNode, "addValue": addDataNode},
