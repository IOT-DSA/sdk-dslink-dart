...
class ConfigSetting {
  ...
  ConfigSetting(this.name, this.type, {this.require: Permission.READ, this.writable: Permission.NEVER, this.defaultValue});
  ConfigSetting.fromMap(this.name, Map m)
      : type = m.containsKey('type') ? m['type'] : 'string',
        require = m.containsKey('require') ? Permission.nameParser[m['require']] : Permission.READ,
        writable = m.containsKey('writable') ? Permission.nameParser[m['writable']] : Permission.NEVER,
        defaultValue = m.containsKey('default') ? m['default'] : null {
  }

  DSError setConfig(Object value, LocalNodeImpl node, Responder responder) {
    if (writable != Permission.NEVER && node.getPermission(responder) > writable) {
...
class Configs {
  ...
  static final ConfigSetting defaultConfig = new ConfigSetting.fromMap('', const {});
